...
const updateTaskAssignees = async (req:Request, res:Response) => {
  console.log("🔥 updateTaskAssignees reached");
...
const addSubTask = async (req:Request, res:Response) => {
  try {
    const { taskId } = req.params;
    const { title } = req.body;
...
const achievedTask = async (req:Request, res:Response) => {
  try {
    const { taskId } = req.params;
...
const deleteTask = async (req: Request, res: Response) => {
  try {
    ...
    // Find the project that owns the task
    const project = await Project.findById(task.project);
    ...
    // Make sure logged-in user belongs to workspace
    const isMember = workspace.members.some(
      (member) =>
