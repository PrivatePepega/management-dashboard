...
import WorkspaceInvite from "../model/workspace-invite.js";
...
interface WorkspaceInviteTokenPayload extends jwt.JwtPayload {
  user: string;
  workspaceId: string;
  role: string;
}
...
const getWorkspaceProjects = async (req:Request <{ workspaceId: string }>, res:Response) => {
  try {
    const { workspaceId } = req.params;
...
const getWorkspaceStats = async (req:Request <{ workspaceId: string }>, res:Response) => {
  try {
    const { workspaceId } = req.params;
    ...
    for (const project of projects) {
      for (const task of project.tasks) {
        ...
        if (dayInDate !== -1) {
          const date = last7Days[dayInDate];
    ...
    for (const project of projects) {
      const status = projectStatusData.find(
        (item) => item.name === project.status
      );
...
const inviteUserToWorkspace = async (req:Request <{ workspaceId: string }>, res:Response) => {
  try {
    const { workspaceId } = req.params;
...
const acceptGenerateInvite = async (req:Request, res:Response) => {
  try {
    const { workspaceId } = req.params;
...
const acceptInviteByToken = async (req:Request, res:Response) => {
  try {
    ...
    const payload = decoded as WorkspaceInviteTokenPayload;
    
    const { user, workspaceId, role } = payload;
